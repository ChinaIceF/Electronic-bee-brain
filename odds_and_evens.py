...
class Neural_Network(object):
    ...
    def train(self, training_set, val_set):
        ...
        for type_index in range(len(training_set)):                                   #  遍历训练集中每一类
            ...
            qqqqq = expected_result * 2 - 1
            qqqqq = numpy.array([[qqqqq[0]],[qqqqq[1]]])
            #print(qqqqq)
...
if __name__ == "__main__":
    ...
    input_value, levels_weights, levels_bias = generate_i_w_b([20, 15, 2])
    each_type_name = ["Odds 奇数", "Evens 偶数"]
    
    print("  Loading training set..")
    train_each_type_set = [[1, 3, 7, 9, 13,   17, 19, 23,27, 31,   33, 39, 45, 47, 49,   55, 51, 53, 63, 65,   69, 71, 79, 89, 85,   91, 95, 97],
                                        [2, 6, 8, 12, 14,   18, 22, 26, 30, 32,   36, 38, 42, 48, 50,   52, 54, 58, 62, 64,   68, 70, 74, 78, 80,  92, 96, 98]]
    train_each_type_set = [[1,3,5,7,9,11],
                                        [0,2,4,6,8,10]]
    
    
    val_each_type_set = [list(range(1,100,2)), list(range(0,100, 2))]
    
    train_each_type_set = [number_to_array(train_each_type_set[0]), number_to_array(train_each_type_set[1])]
    val_each_type_set = [number_to_array(val_each_type_set[0]), number_to_array(val_each_type_set[1])]
    
    #print(number_to_array(train_each_type_set[0]))

    
    # train_each_type_set = [load_set("./训练集/squares/"), load_set("./训练集/circles/")]
    # val_each_type_set = [load_set("./验证集/squares/"), load_set("./验证集/circles/")]
